...
from .pam import InceptionB
from .MPAM import MultilPAM
from .SPFusion import Fusion
...
class FPN(Backbone):
    ...
    def forward(self, x):
        ...
        selayer = MultilPAM(256)
        selayer = selayer.to(torch.device('cuda'))
        results[4] = selayer(results[4])
        
        fusion = FusionLayer(256)
        fusion = fusion.to(torch.device('cuda'))
        results[0]=fusion(results,0)
...
class FusionLayer(nn.Module):
    def __init__(self, channel):
        ...
        self.branch = nn.Conv2d(channel,64, kernel_size=1)
    ...
    def forward(self, x,idx):
        ...
        for i, f in enumerate(x):
            ...
            if i!=0 :
                ...
                sample = nn.functional.interpolate(branch,[8,8])
                k,c,h,w = sample.size()
                gcn1 = gcn(c,c,k,h,w)
                gcn1 = gcn1.to(torch.device('cuda'))
                sample = gcn1(sample)
                sample = nn.functional.interpolate(sample,[a,b])
            else :
                sample = nn.functional.interpolate(branch,[a,b])
...
def build_retinanet_dla_fpn_backbone(cfg, input_shape: ShapeSpec):
    ...
    in_features = cfg.MODEL.FPN.IN_FEATURES
    out_channels = cfg.MODEL.FPN.OUT_CHANNELS
    in_channels_p6p7 = bottom_up.out_feature_channels["level5"]
    backbone = FPN(
        bottom_up=bottom_up,
        in_features=in_features,
        out_channels=out_channels,
